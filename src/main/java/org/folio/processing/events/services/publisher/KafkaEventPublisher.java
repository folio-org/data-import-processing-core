...
public class KafkaEventPublisher implements EventPublisher {
  ...
  public CompletableFuture<Event> publish(DataImportEventPayload eventPayload) {
    ...
    String correlationId = eventPayload.getCorrelationId();
    ...
    try {
      Event event = new Event()
        ...
        .withEventMetadata(new EventMetadata()
          ...
          .withEventTTL(1)
          .withPublishedBy(PomReaderUtil.INSTANCE.constructModuleVersionAndVersion(
      ...
      producer.write(kafkaRecord, war -> {
        ...
        if (war.succeeded()) {
          LOGGER.info("Event with type: '{}' by jobExecutionId: '{}', correlationId: '{}' and recordId: '{}' with chunkId: '{}' was sent to the topic '{}' ",
            eventType, jobExecutionId, correlationId, recordId, chunkId, topicName);
          future.complete(event);
        } else {
          Throwable cause = war.cause();
          LOGGER.error("{} write error for event: '{}' by jobExecutionId: '{}', correlationId: '{}' with recordId: '{}' and with chunkId: '{}' ",
            producerName, jobExecutionId, correlationId, eventType, recordId, chunkId, cause);
          future.completeExceptionally(cause);
        }
      });
    } catch (Exception e) {
      LOGGER.error("Can not publish event: {} with recordId: {}, correlationId: {}",
        eventType, recordId, correlationId, e);
